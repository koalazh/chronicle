...
PROFILES = ("chronicle-seat-a", "chronicle-seat-b", "chronicle-seat-c")
...
def test_fresh_session_title_is_unique(monkeypatch, app_config):
    ...
    session_id = HermesClient(app_config).create_fresh_session(
        PROFILES[0], "profile-key", "a-4-observation"
    )
...
def test_fresh_session_title_is_bounded_for_long_wake_id(monkeypatch, app_config):
    ...
    session_id = HermesClient(app_config).create_fresh_session(
        PROFILES[0], "profile-key", wake_id
    )
...
def test_fresh_session_retries_transient_gateway_failure(monkeypatch, app_config):
    ...
    session_id = HermesClient(app_config).create_fresh_session(
        PROFILES[0], "profile-key", "a-transient-session"
    )
...
def test_fresh_session_does_not_retry_client_failure(monkeypatch, app_config):
    ...
    assert HermesClient(app_config).create_fresh_session(
        PROFILES[0], "profile-key", "a-client-failure"
    ) is None
    assert calls == 1
...
def test_probe_collects_profile_routes_toolsets_and_key_isolation(monkeypatch, app_config):
    ...
    result = probe(app_config, list(PROFILES))

    assert result.health is True
    assert result.profile_status == {profile: 200 for profile in PROFILES}
    assert result.profile_toolsets == {profile: ("memory",) for profile in PROFILES}
